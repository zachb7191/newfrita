...
// Interior pages are shared, so "home" is the first location in the list.
...
function location(content, loc) {
  ...
  const nav = (content.nav || [])
    .map((n) => `<a href="/${esc(n.slug)}">${esc(String(n.title).toLowerCase())}</a>`)
    .join('\n      ');
  const body = `
...
${locationBanner(loc)}
  ...
  const title =
    loc.slug === homeSlug(content)
      ? content.site.title
      : `${content.site.name} ${loc.label} | ${content.site.tagline}`;
...
module.exports = { landing, location, page };
